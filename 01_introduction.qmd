...
- Yes, it is. According to the results of clustering, although the relationship is not a perfect straight line, the presence of distinct clusters indicates that the short-run Phillips curve trade-off is statistically observable.

- Please refer to Model/Method > [Clustering](03_clustering.qmd) for further details.
...
- According to the results of PCA, macroeconomic factors extracted through PCA affect the dynamics of inflation and unemployment mainly through two channels:
...
- Please refer to Model/Method > [PCA](04_pca.qmd) for further details.
...
- The Naïve Bayes model’s performance is highly dependent on the quality of feature engineering and preprocessing, but with appropriate transformations, its accuracy for predicting unemployment and year-over-year inflation improved significantly to 86.4%. In contrast, forecasting month-over-month inflation remained difficult, reflecting the model’s structural limitation of assuming feature independence. Overall, with well-designed features, Naïve Bayes can function as a powerful regime-classification model.

- The Decision Tree model performed exceptionally well for predicting persistent macroeconomic states such as unemployment and year-over-year inflation, achieving high accuracy and stable results. However, its performance declined sharply when forecasting noisy, short-term fluctuations like month-over-month inflation, indicating difficulty in separating signal from noise. Overall, the model’s effectiveness depends heavily on the stability and volatility characteristics of the target time series.

- Please refer to Model/Method > [Naïve Bayes](05_naive_bayes.qmd) and Model/Method > [Decision Trees](06_decision_trees.qmd) for further details.

**6. Does the support vector machine (SVM) improve regime-classification accuracy compared with simpler classifiers such as Naïve Bayes and decision trees?**

- This question cannot be answered because the applications of SVM, regression models, and neural networks were discontinued.
...
- This question cannot be answered within the scope of the current project because no forecasting models or time-series prediction errors were generated, and DM tests require error series from competing models.

**9. According to Granger causality tests, what is the causal direction between inflation and unemployment (inflation → unemployment or unemployment → inflation)?**

- This question cannot be addressed because Granger causality analysis requires VAR estimation and stationarity testing, which were not included in the project.

**10. Through local projection analysis, how do demand shocks and supply shocks dynamically affect inflation and unemployment?**

- This question cannot be evaluated because the project did not perform shock identification or local-projection regressions necessary for tracing dynamic responses to macroeconomic shocks.
